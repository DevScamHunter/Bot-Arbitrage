...
class PolymarketBot {
    ...
    async handleInput(choice: string): Promise<boolean> {
        try {
            ...
            if (requiresAuth && !this.hasPrivateKey) {
                console.log('This action requires a private key. Please add PRIVATE_KEY to .env file.\n');
                return true;
            }
    ...
    async checkBalances(): Promise<void> {
        ...
        console.log('Balance Check (for trading):');
    ...
    async findMarket(): Promise<void> {
        ...
        if (market && market.tokens.length > 0) {
            console.log('Would you like to see price data for this market? (y/n)');
            const answer = await this.prompt('');

            if (answer.toLowerCase() === 'y') {
                for (const token of market.tokens) {
                    console.log(`Fetching data for ${token.outcome}...`);
